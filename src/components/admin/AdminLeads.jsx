...
import { Loader2, RefreshCw, Calculator, FileText, DollarSign, X, Mail, Phone, MapPin, Globe, GraduationCap, DollarSign as Budget, ChevronRight } from 'lucide-react';
...
const AdminLeads = () => {
  ...
  const saveFeedback = async () => {
    ...
    await supabase.from('leads').update({ admin_feedback: feedback }).eq('id', selectedLead.id);
    setLeads(leads.map(l => l.id === selectedLead.id ? { ...l, admin_feedback: feedback } : l));
          ...
          <TableBody>
            {filteredLeads.length === 0 ? (
              ...
              filteredLeads.map((lead) => (
                <TableRow key={lead.id} className="border-b border-slate-800 hover:bg-slate-800/50 cursor-pointer" onClick={() => openDetail(lead)}>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { icon: Phone, label: 'WhatsApp', value: selectedLead.whatsapp },
                  { icon: MapPin, label: 'From', value: selectedLead.origin_country },
                  { icon: Globe, label: 'Target', value: selectedLead.target_destination },
                  { icon: GraduationCap, label: 'Level', value: selectedLead.study_level },
                  { icon: Budget, label: 'Budget', value: selectedLead.budget_range },
                  { icon: ChevronRight, label: 'Intake', value: selectedLead.intake_month },
                  { icon: ChevronRight, label: 'Source', value: selectedLead.source },
              ...
              <div className="flex gap-3">
                ...
                {getPaymentBadge(selectedLead.payment_status)}
                <span className="text-xs text-slate-500 self-center">Score: {selectedLead.score || 0}</span>
              </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Add notes, feedback, or next steps for this lead..."
