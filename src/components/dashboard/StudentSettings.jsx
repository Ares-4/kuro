...
const StudentSettings = () => {
  ...
  const handlePushToggle = async (checked) => {
    ...
    try {
      if (checked) {
        const sub = await subscribeToPush(studentData?.id || user.id);
        setPushEnabled(!!sub);
      } else {
        await unsubscribeFromPush(studentData?.id || user.id);
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      ...
      // 1. Check if student row exists, then update or insert
              ...
              <div className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-800/30 transition-colors">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    <Label className="text-base text-slate-200">Push Notifications</Label>
                  </div>
                  <p className="text-sm text-slate-400">Get notified about new community messages and deadlines, even when the app is closed.</p>
                </div>
                <Switch
                  checked={pushEnabled}
                ...
                {documents.length === 0 ? (
                  ...
                  <div className="grid gap-2">
                    {documents.map((doc, idx) => {
                      const stripped = doc.name.replace(/^\d+_/, '');
                      const isUuid = /^[0-9a-f-]{32,36}$/i.test(stripped.replace(/\.[^.]+$/, ''));
                      const displayName = isUuid ? `Document ${idx + 1}${stripped.includes('.') ? stripped.slice(stripped.lastIndexOf('.')) : ''}` : stripped;
                      const { data: { publicUrl } } = supabase.storage
                        .from('student-documents')
                      ...
                      <div key={doc.id || doc.name} className="flex items-center justify-between p-3 bg-slate-950 rounded-lg border border-slate-800 hover:border-slate-700 transition-colors">
                        <a href={publicUrl} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 overflow-hidden flex-1 min-w-0 group/link">
                          <div className="p-2 bg-blue-900/20 rounded text-blue-400 shrink-0">
                            <FileText className="w-5 h-5" />
                          </div>
                          <div className="min-w-0">
                            <div className="text-sm font-medium text-white truncate max-w-[150px] sm:max-w-[280px] group-hover/link:text-blue-400 transition-colors">{displayName}</div>
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            className="h-8 w-8 p-0 text-slate-500 hover:text-red-400 hover:bg-red-400/10 shrink-0"
